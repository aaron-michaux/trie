...
coverage_html: $(TARGETDIR)/$(TARGET)
	...
	lcov --gcov-tool $(GCOV) -c --directory $(BUILDDIR) --output-file $(TARGETDIR)/app_info_all.info --quiet
	lcov --remove $(TARGETDIR)/app_info_all.info -o $(TARGETDIR)/app_info.info '/usr/*' '*/contrib/*'
	genhtml $(TARGETDIR)/app_info.info --output-directory doc/html/coverage --prefix $(PWD) --title "COVERAGE :: $(TARGET)"
